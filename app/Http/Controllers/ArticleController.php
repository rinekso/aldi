...
class ArticleController extends Controller
{
    ...
    public function input(Request $request){
        ...
        if (@$request->cover != '')
        {
            ...
            if($extension == "jpg") {
                ...
                $form['cover'] = $photoProfileName.'.jpg';
            }else{
    ...
    public function delete($id){
        ...
        $imagePath  = 'assets/images/cover/'.$data->cover;
        File::delete($imagePath);
        app('article')->delete($id);
        return redirect()->back();
    }
    public function search($input){
        $data = app('article')->searchAll($input);
    }
    public function getDetailJson($id){
        $data = app('article')->detail($id);
        return response()->json($data);
    }
    public function getLimitJson($limit){
        $data = app('article')->limit($limit);
        return response()->json($data);
    }
    ...
    public function getPageJson($limit){
        $data = app('article')->getPaginate($limit);
        return response()->json($data);
    }
}
